    ...
    with col1:
        ...
        if review_text.strip():
            ...
            if word_count < MIN_WORDS:
                st.warning(f"⚠️ Review too short ({word_count} words) — model needs at least {MIN_WORDS} words for accurate prediction.")
        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
...
with tab2:
    ...
    sample_csv = pd.DataFrame({"review": [
        ...
        "ఈ సినిమా చూసి time waste అయ్యింది, story logic లేదు, acting కూడా బాగాలేదు",
